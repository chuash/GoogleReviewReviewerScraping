...
reviewfilepath = r'.\entityreviews.csv'
...
contrisumcols = ['Reviewer_ID', 'Name', 'Localguide', 'ScrapedDate','Reviews', 'Ratings','Photos',
                 'Videos', 'Captions', 'Answers', 'Edits',
                 'Reported', 'Places', 'Roads', 'Facts', 'Q&A']
# Field names for contribution details file 
contridetailcols = ['Reviewer_ID', 'ContributionType', 'BusinessName','BusinessAddress',
...
if __name__ == "__main__":
    try:
        ...
        IDlist = Utils.read_ID(reviewfilepath, target, idx)
        ...
        with open(contrisumfilepath, 'a', newline='', encoding='utf-8') as f1, open(contridetailfilepath, 'a', newline='', encoding='utf-8') as f2:
            ...
            for ix in IDlist:
                ...
                time.sleep(1)
                #1)Extract the name
                name = WebDriverWait(driver, 10).until(EC.visibility_of_element_located((By.CLASS_NAME,"geAzIe")))
                
                #2a)Get the breakdown of contributions. First, click to show the contributions
                entry = WebDriverWait(driver, 10).until(EC.visibility_of_element_located((By.CLASS_NAME,"FNyx3")))
                ...
                CT = WebDriverWait(driver, 10).until(EC.visibility_of_all_elements_located((By.CLASS_NAME,"FM5HI")))
                contribtype = [ele.text for ele in CT]
                CC = WebDriverWait(driver, 10).until(EC.visibility_of_all_elements_located((By.CLASS_NAME,"AyEQdd")))
                ...
                if [item.lower() for item in contrisumcols[4:]] == [itemz.lower().split()[0] for itemz in contribtype]:
                
                #2d) Write to contribution summary file
                    contrisum_append.writerow([[ix], name.text, localguide, scrapedatestr] + contribcount)
                else:
                    raise Utils.MyError("Mismatch in contribution types. Cross-check contribution summary file field names with website.")
               
                #2e) Click to close the contribution dialog box
                driver.find_element(By.XPATH,'//*[@id="modal-dialog"]/div/div[2]/div/button/span').click()
                ...
                if len(tabs) == 2:
                    ...
                    raise Utils.MyError("Check the class names for the reviews and photos elements")
                
                #4) First check if there is any review, if no, skip else proceed with scraping
                reviewsection = WebDriverWait(driver, 10).until(EC.visibility_of_element_located((By.CSS_SELECTOR, "[aria-label='Reviews']")))
                if not 'OEnQgb' in [ele.get_attribute('class').strip() for ele in reviewsection.find_elements(By.XPATH, "*")]:
                ...
                #4c) Get the business names and address
                    bizname_add = [ele.text.split('\n') for ele in driver.find_elements(By.CLASS_NAME, "WNxzHc ")]
                    biznames = [ele[0] for ele in bizname_add]
                    bizadd = [' '.join(ele[1:]) if len(ele)>1 else '' for ele in bizname_add]
               
                #4d) Get the business ratings and review dates
                    rating_date_ele = [ele.find_elements(By.XPATH, "*") for ele in driver.find_elements(By.CLASS_NAME, "DU9Pgb")]
                    rating_date = [[item.get_attribute('class').strip() for item in ele] for ele in rating_date_ele]
                    ratings = []
                    reviewdate = []
                    for i in range(len(rating_date_ele)):
                        [ratings.append(rating_date_ele[i][0].get_attribute("aria-label")) if "kvMYJc" in rating_date[i] else ratings.append('')]
                        [reviewdate.append(rating_date_ele[i][1].text) if "rsqaWe" in rating_date[i] else reviewdate.append('')]
                ...
                #4f) Write to contribution details file
                    for i in range(len(biznames)):
                        contridetail_append.writerow([[ix],'Review',biznames[i],
                                                      bizadd[i],ratings[i], Utils.datediff(scrapedate,reviewdate[i]),
                                                      scrapedatestr,reviews[i]])
                else:
                    contridetail_append.writerow([[ix],'Review','','','','',scrapedatestr,''])
                ...
                photosection = WebDriverWait(driver, 10).until(EC.visibility_of_element_located((By.CSS_SELECTOR, "[aria-label='Photos']")))
                if not 'OEnQgb' in [ele.get_attribute('class').strip() for ele in photosection.find_elements(By.XPATH, "*")]:
                
                #5b) Scroll to bottom of photos page so as to show all photos
                    Utils.scroll_to_bottom(photosection)
                ...
                #5d) Write to contribution details file
                    for i in range(len(biznames)):
                        contridetail_append.writerow([[ix],'Photos',biznames[i],
                                                  bizadd[i],'', '',
                                                  scrapedatestr,''])
                else:
                    contridetail_append.writerow([[ix],'Photos','','','','',scrapedatestr,''])
